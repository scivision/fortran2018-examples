        ...
        for line in f:
            if line.startswith("CMAKE_C_COMPILER:FILEPATH"):
                ...
                if not compilers["CC"] in cc:
                    wipe = True
                    break
            elif "FC" in compilers and line.startswith(
...
def cmake_setup(
    ...
    print(" ".join(cmd))
    ret = subprocess.run(cmd, cwd=build, env=os.environ.update(compilers))
    if ret.returncode:
    ...
    if dotest:
        ...
        if compilers["CC"] == "cl":
            ret = subprocess.run(
                [cmake_exe, "--build", str(build), "--target", "RUN_TESTS"]
            )
        else:
            ret = subprocess.run(
                [ctest_exe, "--parallel", "--test-load", "4", "--output-on-failure"],
                cwd=build,
            )
    ...
    if install is not None:  # blank '' or ' ' etc. will use dfault install path
        ret = subprocess.run(
            [cmake_exe, "--build", str(build), "--parallel", "--target", "install"]
        )
        if ret.returncode:
            raise SystemExit(ret.returncode)
...
def msvc_params() -> Tuple[Dict[str, str], List[str]]:
    if not shutil.which("cl"):
        raise EnvironmentError(
            "Must have PATH set to include MSVC cl.exe compiler bin directory e.g. by vcvarsall.bat"
        )
...
if __name__ == "__main__":
    p = ArgumentParser()
    p.add_argument(
        "vendor", help="compiler vendor", choices=["clang", "gnu", "intel", "msvc"]
    )
    p.add_argument("-wipe", help="wipe and rebuild from scratch", action="store_true")
    p.add_argument("-i", "--implementation", help="which LAPACK implementation")
    p.add_argument(
        "-n", "--no-test", help="do not run self-test / example", action="store_false"
    )
